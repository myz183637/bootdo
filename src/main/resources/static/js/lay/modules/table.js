...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-so', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'<div class="layui-table-header">'
      ,TPL_HEADER()
    ,'</div>'
    ,'<div class="layui-table-body layui-table-so">'
      ,TPL_BODY
    ,'</div>'
    ...
    that.layBody.on('click', 'td', function(){
      ...
      if(elemCell.prop('scrollWidth') > elemCell.outerWidth()){
        that.tipsIndex = layer.tips([
          '<div class="layui-table-tips-so" style="margin-top: -'+ (elemCell.height() + 16) +'px;'+ function(){
            if(config.size === 'sm'){
